...
class StrategyRegistry:
    """Registry for ingestion pipeline strategies."""
    ...
    def get(cls, stage: str, name: str, **kwargs) -> Any:
        ...
        if not stage_strategies:
            raise ValueError(
                f"No strategies registered for stage '{stage}'. "
                f"Available stages: {list(cls._strategies.keys())}"
            )
        strategy_cls = stage_strategies.get(name)
        if not strategy_cls:
            raise ValueError(
                f"Strategy '{name}' not found for stage '{stage}'. "
                f"Available: {list(stage_strategies.keys())}"
            )
